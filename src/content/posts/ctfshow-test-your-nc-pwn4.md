...
![程序逻辑](/alf-ovo-blog/attachments/ctfshow/test_your_nc/pwn4/01-logic.png)

![关键比较](/alf-ovo-blog/attachments/ctfshow/test_your_nc/pwn4/02-compare.png)

当 `s1 == s2` 时会 getshell。`s1` 是 `CTFshowPWN`，`s2` 是读取的用户输入，所以直接输入 `CTFshowPWN` 即可获取 flag。

![拿到 flag](/alf-ovo-blog/attachments/ctfshow/test_your_nc/pwn4/03-flag.png)
